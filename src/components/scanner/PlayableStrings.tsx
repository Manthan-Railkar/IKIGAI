...
interface PlayableStringsProps {
...
const DEFAULT_INSTRUMENT: Instrument = {
  ...
  description: "A long-necked plucked lute with sympathetic strings.",
  historical_context: "The quintessential voice of Hindustani classical music.",
...
export default function PlayableStrings({
  ...
  // Preload instrument audio samples on mount
  useEffect(() => {
  ...
  const handlePluck = useCallback(
    (swara: SwaraDefinition, index: number) => {
      // Play real sampled instrument note
      sampleEngine.playNote(instrument.id, swara.name, swara.frequency, 2.2, 0.9);
      setActiveNote(swara);
  ...
  // Synchronize with external highlight (playback of curated raga or recording)
  useEffect(() => {
    ...
    <div className={`relative w-full max-w-2xl mx-auto select-none ${className}`}>
      ...
      <div className="flex items-center justify-between px-3.5 py-1.5 sm:px-4 sm:py-2.5 mb-2.5 sm:mb-4 rounded-xl border border-white/10 bg-[#15181e]/80 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-[#ffc75a] animate-pulse" />
        ...
        {activeNote ? (
          ...
          <span className="text-[11px] sm:text-xs font-grotesque text-white/40 italic">
            Pluck a string to play
          </span>
        )}
      </div>

      {/* Cinematic Visual Backdrop + String Overlay */}
      <div className="relative rounded-2xl border border-white/15 bg-gradient-to-b from-[#181b22] to-[#0d0f14] overflow-hidden shadow-2xl p-3 sm:p-5 md:p-6">
        ...
        <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden">
          <Image
            src={instrument.image_url || "/Assets/veena_reveal.jpg"}
            alt={instrument.name}
            fill
        ...
        <div className="relative z-10">
          ...
          <div className="flex justify-between items-center text-[9px] sm:text-[10px] font-mono text-white/40 uppercase tracking-widest mb-2 sm:mb-3 px-1">
            ...
            <span className="hidden sm:inline">{instrument.name} · Real Plucked Acoustic Samples</span>
